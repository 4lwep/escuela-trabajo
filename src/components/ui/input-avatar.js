...
function InputAvatar({ name = "image", user }) {
  const [selectedAvatar, setSelectedAvatar] = useState(
    user?.image || "/images/avatar-80.png",
  );
  const [avatarOpen, setAvatarOpen] = useState(false);

  const avatares = Array.from(
    { length: 80 },
    (_, index) => `/images/avatar-${String(index).padStart(2, "0")}.png`,
  );
    ...
    <div className="relative flex flex-col gap-8">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          />
          <ChevronDown
            className={`w-5 h-5 text-gray-500 transition-transform ${avatarOpen ? "rotate-180" : ""}`}
          />
        </div>
      </button>
